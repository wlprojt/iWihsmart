...
struct TodaysBestDealSection: View {
    @StateObject private var vm = HomeDealsViewModel()
    ...
    var body: some View {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 16) {
                        ForEach(vm.products) { p in
                            Button { onProductTap(p) } label: {
                                BestDealCard(product: p)
                            }
                            .buttonStyle(.plain)
                        }
                    }
        ...
        .task { await vm.fetchBestDeals() }
    }
}

private struct BestDealCard: View {
    let product: Product
    ...
    private func productImageURL(_ product: Product) -> URL? {
        ...
        // ✅ Case 1: Cloudinary or any absolute URL
        if first.hasPrefix("http://") || first.hasPrefix("https://") {
            return URL(string: first)
        }

        // ✅ Case 2: Relative path from backend (/uploads/...)
